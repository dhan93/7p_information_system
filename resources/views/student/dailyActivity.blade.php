          ...
          @foreach ($userActivities as $userActivity)
            <tr class="bg-white border-b-2 border-gray-200">
              ...
              <td class="py-2">
                ...
                <a href="{{route('daily_activity.edit', $userActivity->id)}}" class="px-2 py-1 text-gray-600 border border-gray-400 rounded-md hover:bg-gray-300 hover:border-gray-300">
          ...
          @foreach ($activityGroup as $group)
              ...
              @foreach ($group->activities as $activity)
                <tr>
                  ...
                  <td class="px-2 py-1 border">{!!var_dump($matrix[$loop->index]['date'])!!}</td>
                  @for ($j = 1; $j <= 15; $j++)
                    <td class="text-center border">
                      {!!$matrix[$loop->index]['date'][$j]!!}
                      {{$loop->parent->index}}
                    </td>
                  @endfor
